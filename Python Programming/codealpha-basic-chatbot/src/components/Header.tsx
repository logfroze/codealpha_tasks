...
import { Terminal, MessageSquare, FileCode, CheckCircle2, RotateCcw, Download } from 'lucide-react';
        ...
        <div className="flex items-center flex-wrap gap-2 w-full sm:w-auto justify-between sm:justify-end">
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              title="Restart session (clears conversation)"
            ...
            <button
              id="download-chatbot-py-btn"
              onClick={() => onDownloadFile('chatbot.py')}
              title="Download chatbot.py"
              className="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium text-zinc-700 bg-white hover:bg-zinc-50 border border-zinc-200 rounded-md transition-colors shadow-xs"
            >
              <Download className="w-3.5 h-3.5" />
              <span className="hidden md:inline">Download .py</span>
            </button>
          </div>
        </div>
      </div>
